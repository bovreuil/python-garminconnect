...
function getDayHRTimeSeries(dayData) {
    ...
    // dayData.heart_rate_values is already in [[timestamp, hr_value], ...] format
    ...
    // Filter out null/invalid values and ensure proper format
    const cleanSeries = hrSeries
        ...
        .map(point => [point[0], point[1]]); // Ensure [timestamp, value] format
...
function getActivitySpO2TimeSeries(activity) {
    ...
    // Check for manual SpO2 entries first
